...
Particle::Particle(double x_, double y_,
                   double e_, double r_, double maxE)
  : x(x_), y(y_), energy(std::clamp(e_,0.0,maxE)),
    radius(r_), maxEnergy(maxE)
{}
...
{ std::lock_guard g(mtx); return energy; }
...
{
    const Particle *a=this < &o? this:&o, *b=this < &o? &o:this;
    std::scoped_lock lock(a->mtx, b->mtx);
    double dx=x-o.x, dy=y-o.y, rsum=radius+o.radius;
...
void Particle::collide(Particle& o)
{
    ...
    std::swap(vx,o.vx); std::swap(vy,o.vy);
